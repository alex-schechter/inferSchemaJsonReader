...
public class JsonInferRowRecordReader extends AbstractJsonRowRecordReader {
    private final RecordSchema schema;
    private final ComponentLog logger;


    public JsonInferRowRecordReader(InputStream in, ComponentLog logger, RecordSchema schema, String dateFormat, String timeFormat, String timestampFormat) throws IOException, MalformedRecordException {
        super(in, logger, dateFormat, timeFormat, timestampFormat);
        this.schema = schema;
        logger.info("is the constructor");
        this.logger = logger;
    }
    ...
    private Record convertJsonNodeToRecord(final JsonNode jsonNode, final RecordSchema schema, final boolean coerceTypes, final boolean dropUnknown, final String fieldNamePrefix)
        ...
        logger.info("line 70");
        if (jsonNode == null) {
            logger.info("line 72 the json node is null");
            return null;
        }
    ...
    private Record convertJsonNodeToRecord(final JsonNode jsonNode, final RecordSchema schema, final String fieldNamePrefix,
        ...
        logger.info("line 99");
        logger.info("the schema is: " + schema.toString());
